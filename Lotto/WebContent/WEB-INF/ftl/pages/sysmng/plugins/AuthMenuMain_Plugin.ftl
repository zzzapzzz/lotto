		...
		<script type="text/javascript">
			...
			function initPlugin() {
				...
				jQuery("#jqgrid").jqGrid({
					...
					onSelectRow: function(id){		/** 로우 선택시 함수 **/
						...
						getAuthMenuInfoList(rowdata.auth_cd);
					},
					autowidth : true
			...
			function getAuthMenuInfoList(auth_cd){
				...
				$.ajax({
					type: "POST",
					url: "${APP_ROOT}/sysmng/authMenuInfoList.do",
					data : param,
					dataType: "json",
					async: false,
		    ...
		    function saveMenuAuthInfo(){
				...
				if (checkedNodes.length > 0) {
					for (var i = 0 ; i < checkedNodes.length ; i++) {
						if (checkedNodes[i].indexOf('C')== 0 && checkedNodes[i].indexOf('1')== 1) {
							...
							if(i > 0){
								tmp_menu_id = tmp_menu_id + "@";
							}							
							tmp_menu_id = tmp_menu_id + checkedNodes[i];
		    	...
		    	$.ajax({
					type: "POST",
					url: "${APP_ROOT}/sysmng/saveMenuAuthInfo.do",
					data: param,
					dataType: "json",
					async: false,
					contentType: "application/x-www-form-urlencoded; charset=UTF-8",
